...
def ListaRepetidos(lista):
    ...
    if type(lista) != list:
        ...
        for s in lista:
            Repeticiones[s] = lista.count(s)
        ListaRepetidos = [(k, v) for k, v in Repeticiones.items()]
...
def OrdenarDiccionario(diccionario_par, clave, descendente=True):
    ...
    '''
    #Tu código aca:
    if not isinstance(diccionario_par, dict) or clave not in diccionario_par.keys():
        return None

    claves = list(diccionario_par.keys())
    valores = list(diccionario_par.values())

    valores_clave = valores[claves.index(clave)]

    indices_ordenados = sorted(range(len(valores_clave)), key=lambda i: valores_clave[i], reverse=descendente)

    diccionario_ordenado = {k: [] for k in claves}
    for i in indices_ordenados:
        for j in range(len(claves)):
            diccionario_ordenado[claves[j]].append(valores[j][i])
